    ...
    html += `
      <div class="source-card">
        ...
        <div class="source-content">
          ...
          ${vt.reputation ? `<p><strong>Reputation:</strong> ${vt.reputation.score}/${vt.reputation.maxScore}</p>` : ''}
          ...
          ${vt.lastAnalysis ? `<p><strong>Last Analysis:</strong> ${vt.lastAnalysis}</p>` : ''}
          ${vt.detection?.summary ? `<p><strong>Detection:</strong> ${vt.detection.summary}</p>` : ''}
    ...
    html += `
      <div class="source-card">
        ...
        <div class="source-content">
          ...
          ${abuse.abuseConfidence !== null ? `<p><strong>Abuse Confidence:</strong> ${abuse.abuseConfidence}%</p>` : ''}
          ${abuse.isPublic !== null ? `<p><strong>Public IP:</strong> ${abuse.isPublic ? 'Yes' : 'No'}</p>` : ''}
          ${abuse.isWhitelisted ? `<p><strong>Whitelisted:</strong> Yes</p>` : ''}
          ${abuse.usageType ? `<p><strong>Usage Type:</strong> ${abuse.usageType}</p>` : ''}
          ...
          ${abuse.lastReported ? `<p><strong>Last Reported:</strong> ${abuse.lastReported}</p>` : ''}
          ${abuse.reports && abuse.reports.length > 0 ? `<p><strong>Recent Reports:</strong> ${abuse.reports.length} found</p>` : ''}
  ...
  if (data.errors && data.errors.length > 0) {
    html += `
      <div class="error-card">
        <h3>⚠️ Errors</h3>
        <ul>
          ${data.errors.map(err => `<li>${err.source}: ${err.error}</li>`).join('')}
        </ul>
      </div>
    `;
  }
